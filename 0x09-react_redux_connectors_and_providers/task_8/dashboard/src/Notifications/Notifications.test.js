...
describe('Notifications filter buttons', () => {
  ...
  it('calls setNotificationFilter with URGENT when ‼️ button is clicked', () => {
    const wrapper = shallow(<Notifications {...defaultProps} />);
    wrapper.find('button').at(1).simulate('click');
    expect(setNotificationFilter).toHaveBeenCalledWith('URGENT');
  });

  it('calls setNotificationFilter with DEFAULT when 💠 button is clicked', () => {
    const wrapper = shallow(<Notifications {...defaultProps} />);
